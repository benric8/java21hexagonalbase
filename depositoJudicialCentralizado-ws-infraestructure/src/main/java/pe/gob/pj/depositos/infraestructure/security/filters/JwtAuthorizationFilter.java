package pe.gob.pj.depositos.infraestructure.security.filters;
...
import pe.gob.pj.depositos.domain.common.enums.Claim;
import pe.gob.pj.depositos.domain.common.utils.ProjectConstants;
...
import pe.gob.pj.depositos.domain.exceptions.TokenException;
import pe.gob.pj.depositos.domain.model.auditoriageneral.PeticionServicios;
...
public class JwtAuthorizationFilter extends BasicAuthenticationFilter {
  ...
  /**
   * Descripción: Obtiene la autenticación desde el token.
   *
   * @param request la petición HTTP de tipo {@link HttpServletRequest}
   * @return {@link UsernamePasswordAuthenticationToken} - Información de autenticación proveniente del token
   * @throws Exception captura una excepción genérica
   */
  ...
  private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request) {
    ...
    if (!ProjectUtils.isNullOrEmpty(token) && token.startsWith(SecurityConstants.TOKEN_PREFIX)) {
      try {
        ...
        log.warn("{} Request to parse expired JWT : {} failed : {}", cuo,token, exception.getMessage());
      }
    }
    return null;
  }
